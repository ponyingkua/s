...
Styling & helper dari chart.py; indikator (EMA 20/50/200, S/R, swing)
dari hasil analyze.py. Ukuran panel & jumlah candle diselaraskan
dengan build_multi_tf_card di chart.py.
"""
...
def _draw_analyze_indicators(ax, df: pd.DataFrame, n_show: int, tf_info: dict,
    ...
    ax.plot(x, ema20, color=EMA20_COLOR, linewidth=0.9, alpha=0.92,
            zorder=4, solid_capstyle="round")
    ax.plot(x, ema50, color=EMA50_COLOR, linewidth=0.9, alpha=0.92,
            zorder=4, solid_capstyle="round")
    if len(df) >= 200:
        ema200 = close.ewm(span=200, adjust=False).mean().tail(n_show).to_numpy()
        ax.plot(x, ema200, color=EMA200_COLOR, linewidth=0.9, alpha=0.92,
                zorder=4, solid_capstyle="round")
    ...
    for idx, val in sh:
        if idx >= offset:
            ax.scatter(idx - offset, val * 1.004, marker="v",
                       color=chart.DOWN, s=11, alpha=0.75, zorder=5)
    for idx, val in sl:
        if idx >= offset:
            ax.scatter(idx - offset, val * 0.996, marker="^",
                       color=chart.UP, s=11, alpha=0.75, zorder=5)
...
def build_mtfk_chart(
    ...
    dpi = 200
    fig_w = width_px / dpi
    # Samakan dengan build_multi_tf_card di chart.py
    ...
    for idx, tf in enumerate(valid_tfs):
        ...
        # Samakan jumlah candle dengan chart.py multi-card (≈ setengah)
        ...
        # Samakan proporsi volume dengan chart.py multi-card
        ...
        ax_p.set_title(
            ...
            color=badge_color, fontsize=title_fs, fontweight="bold", loc="left", pad=6,
        )

        dec = chart.decimals_from_price(float(plot_df["close"].iloc[-1]))
        last_price = chart.format_price(plot_df["close"].iloc[-1], dec)
        ax_p.text(
            0.99, 0.03, last_price, transform=ax_p.transAxes, color=chart.TEXT,
    ...
    fig.text(
        ...
        fontsize=header_fs, fontweight="bold", color=chart.TEXT, ha="left", va="top",
    )
