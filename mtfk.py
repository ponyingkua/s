...
Arah dependensi: mtfk.py hanya mengimpor _swing_points dari analyze
secara lazy (di dalam fungsi), bukan di level modul. Import
"from mtfk import build_mtfk_chart" di analyze.py juga HARUS tetap lazy
(di dalam main(), bukan di atas file) -- kalau tidak, circular import
lagi seperti sebelumnya (analyze -> mtfk -> chart -> analyze).
"""
...
# JANGAN import apa pun dari analyze di level modul.
# chart.py juga mengimpor analyze, dan analyze (lewat main) mengimpor mtfk.
# Import level-modul dari analyze di sini akan memicu circular import
# (partially initialized module). _swing_points diimpor lazy di dalam
# _draw_analyze_indicators saja.
...
def _draw_analyze_indicators(ax, df: pd.DataFrame, n_show: int, tf_info: dict,
    ...
    _swing_points juga diimpor di sini (lazy) agar tidak terjadi circular
    import saat modul mtfk di-load (chart.py mengimpor analyze, analyze
    memanggil mtfk dari main).
    """
